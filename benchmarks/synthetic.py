...
COMMON_PORTS = [22, 25, 53, 80, 110, 143, 443, 465, 587, 993, 995, 3306, 5432, 6379, 8080, 8443, 9200, 27017]
...
def generate_policy(size: int, seed: int = 0) -> NetworkPolicy:
    ...
    for i in range(size):
        ...
        rule.protocol = Protocol(name=rng.choice(PROTOCOLS))
        rule.from_ip(_random_network(rng))
        rule.port(COMMON_PORTS[i % len(COMMON_PORTS)])
...
def generate_device_lines(size: int, seed: int = 0) -> List[str]:
    ...
    for i in range(size):
        ...
        lines.append(
            f"-A INPUT -p {proto} -s {src} --dport {port} -j {action}"
        )
    return lines
